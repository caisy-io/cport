...
export const writeTags = async (tags: { sys: { id: string }; name: string }[]) => {
...
export const normalizeContentfulTag = (tag: { sys: { id: string }; name: string }): z.infer<typeof tagSchema> => {
  return {
    id: tag.sys.id,
    name: tag.name,
...
function stringToColorCode(inputString: string): string {
  let hash = 0;
